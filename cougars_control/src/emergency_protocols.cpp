...
public:
  ...
  EmergencyProtocols() : Node("emergency_protocols") {
    ...
    depth_subscription_ = this->create_subscription<
        ...
        "depth_data", 10,
        std::bind(&EmergencyProtocols::depth_callback, this, _1));
    leak_subscription_ =
        this->create_subscription<sensor_msgs::msg::FluidPressure>(
            "leak/data", 1,
            std::bind(&EmergencyProtocols::leak_callback, this, _1));
    imu_subscription_ = this->create_subscription<sensor_msgs::msg::Imu>(
        "modem_imu", 1, std::bind(&EmergencyProtocols::imu_callback, this, _1));
    battery_subscription_ =
        this->create_subscription<sensor_msgs::msg::BatteryState>(
            "battery/data", 1,
            std::bind(&EmergencyProtocols::battery_callback, this, _1));
    gps_subscription_ = this->create_subscription<gps_msgs::msg::GPSFix>(
        "extended_fix", 1,
        std::bind(&EmergencyProtocols::gps_fix_callback, this, _1));
    dvl_subscription_ = this->create_subscription<dvl_msgs::msg::DVLDR>(
        "dvl/position", 1,
    ...
    //         qos,std::bind(&EmergencyProtocols::factor_graph_callback, this,
    //         _1));
    // vehicle_status_moos_subscription_ =
    // this->create_subscription<cougars_interfaces::msg::VehicleStatus>("vehicle_status",
    //         qos, std::bind(&EmergencyProtocols::moos_status_callback, this,
    //         _1));
    ...
    status_publisher_ =
        this->create_publisher<cougars_interfaces::msg::SystemStatus>(
            "safety_status", 10);
    ...
    //      * too deep
    //      * TODO: add more as needed
    surface_fin_override_request =
        std::make_shared<std_srvs::srv::SetBool::Request>();
    surface_fin_override_client =
...
private:
  void surface() {
    if (this->init) {
      ...
      send_set_bool_req(surface_fin_override_request,
                        surface_fin_override_client);
    }
  }
  ...
  bool update_publishers() {
    ...
    for (auto itr0 : topics_publishing) {
      ...
      if (!topic_has_publishers) {
        ...
        this->okay = false;
        RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "%s has no publishers!",
                    itr0.first.c_str());
      }
    }
  ...
  // this timer looks through all topics on the network and checks for crucial
  // topics such as smoothed_output, leak/data, etc. Key topics are in
  // topics_publishing map
  void monitor_timer_callback() {
    ...
    if (counter < 1) { // dont run the first time
      ...
      if (latest_dvl_message.header.stamp.sec !=
          0) { // checking if this is a real message, not just default
        if (((this->get_clock()->now().seconds() -
              latest_dvl_message.header.stamp.sec) >
             this->get_parameter("dvl_message_timeout").as_int())) {
          this->okay = false;
          message.dvl_status.set__data(message.dvl_status.data | 1);
        }
        if (latest_dvl_message.pos_std >
            this->get_parameter("dvl_position_stddev_threshold").as_double()) {
          this->okay = false;
          message.dvl_status.set__data(message.dvl_status.data | (1 >> 1));
        }
      }
      if (latest_modem_message.header.stamp.sec !=
          0) { // checking if this is a real message, not just default
        if (((this->get_clock()->now().seconds() -
              latest_modem_message.header.stamp.sec) >
             this->get_parameter("modem_message_timeout").as_int())) {
          this->okay = false;
      ...
      if (latest_gps_message.status.satellites_used <
          (this->get_parameter("gps_sat_num_threshold").as_int())) {
        // not going to set okay to false because this is expected during the
        // mission
  ...
  void handle_gps_param_response(
    ...
    this->gps_origin_lat =
        response->values[0].double_value; // test which value is which
  ...
  void grab_gps_params() {
    ...
    auto future = client_->async_send_request(
        request, std::bind(&EmergencyProtocols::handle_gps_param_response, this,
                           std::placeholders::_1));
  }
  void gps_fix_callback(const gps_msgs::msg::GPSFix &msg) {
    if (this->get_parameter("gps_origin_check").as_bool()) {
      if (this->gps_count < this->get_parameter("origin_gps_msgs").as_int()) {
        this->gps_count++;
        if (gps_origin_lat != 0 && gps_origin_lon != 0 &&
            gps_origin_alt != 0) { // params grabbed
          if (abs(msg.longitude - gps_origin_lon >
                  this->get_parameter("origin_gps_threshold").as_double()) ||
              abs(msg.altitude - gps_origin_alt >
                  this->get_parameter("origin_gps_threshold_alt").as_int()) ||
              abs(msg.latitude - gps_origin_lat) >
                  this->get_parameter("origin_gps_threshold").as_double()) {
            RCLCPP_ERROR(this->get_logger(),
                         "GPS origin likely set incorrectly");
  ...
  void imu_callback(const sensor_msgs::msg::Imu &msg) { this->imuPub = true; }
  ...
  void
  depth_callback(const geometry_msgs::msg::PoseWithCovarianceStamped &msg) {
    if (this->get_parameter("monitor_depth").as_bool()) {
      ...
      if (depth_val < this->get_parameter("deepest_safe_depth").as_double()) {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Emergency request being sent in response to leak");
        surface_then_disarm();
      }
  ...
  void battery_callback(const sensor_msgs::msg::BatteryState &msg) {
    ...
    if (this->get_parameter("monitor_low_battery").as_bool()) {
      ...
      if (curr_volt < this->get_parameter("critical_voltage").as_double()) {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Emergency request being sent in response to voltage");
        surface_then_disarm();
      }
  ...
  void
  send_set_bool_req(std::shared_ptr<std_srvs::srv::SetBool::Request> request,
  ...
  //   rclcpp::Subscription<cougars_interfaces::msg::VehicleStatus>::SharedPtr
  //   vehicle_status_moos_subscription_;
  ...
  // publisher
  rclcpp::Publisher<cougars_interfaces::msg::SystemStatus>::SharedPtr
      status_publisher_;
