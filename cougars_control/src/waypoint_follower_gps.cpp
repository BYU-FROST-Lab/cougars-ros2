...
public:
  WaypointFollowerGpsNode()
      : Node("waypoint_follower_gps"), current_waypoint_index_(0),
    ...
    RCLCPP_INFO(this->get_logger(), " - Waypoint file: %s",
                waypoint_file_path_.c_str());
    RCLCPP_INFO(this->get_logger(), " - Slip radius: %.2f m", slip_radius_);
    RCLCPP_INFO(this->get_logger(), " - Desired speed: %.2f",
                desired_travel_speed_);
    RCLCPP_INFO(this->get_logger(),
                " - Using /gps_odom for position and /modem_imu for heading.");
    ...
    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "gps_odom", 10,
        std::bind(&WaypointFollowerGpsNode::odom_callback, this,
                  std::placeholders::_1));

    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        "modem_imu", 10,
        std::bind(&WaypointFollowerGpsNode::imu_callback, this,
                  std::placeholders::_1));
    ...
    if (load_waypoints()) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "Mission loaded successfully with %zu waypoints.",
                  waypoints_.size());
    } else {
      RCLCPP_ERROR(this->get_logger(),
                   "Failed to load mission. Node will not navigate.");
    }
...
private:
  ...
  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    // RCLCPP_DEBUG(this->get_logger(), "GPS Odom Received: X=%.2f, Y=%.2f",
  ...
  void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    // RCLCPP_DEBUG(this->get_logger(), "Current Heading (Degrees ENU): %.2f",
    // current_heading_degrees_);
  }
  ...
  void control_loop_callback() {
    if (!mission_loaded_ || !mission_active_ || waypoints_.empty()) {
      ...
      publish_control_commands(
          current_heading_degrees_,
          waypoints_.empty() ? 0.5 : waypoints_.back().depth, 0.0);
      return;
    }
    ...
    if (current_waypoint_index_ >= waypoints_.size()) {
      ...
      publish_control_commands(current_heading_degrees_,
                               waypoints_.back().depth, 0.0); // Stop
      return;
    }
    ...
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                         "GPS: Pos(%.2f, %.2f) | Target WP %d (%.2f, %.2f) | "
                         "Dist: %.2f | Head: %.1f",
                         current_x_, current_y_, target_wp.id, target_wp.enu_x,
                         target_wp.enu_y, distance_to_target,
                         current_heading_degrees_);
    ...
    if (distance_to_target < slip_radius_) {
      ...
      // If that was the last one, the check at the start of the loop will
      // handle it next time.
    ...
    double desired_heading_rad = calculate_bearing(
        current_x_, current_y_, next_target_wp.enu_x, next_target_wp.enu_y);
    double desired_heading_deg =
        normalize_angle_degrees(desired_heading_rad * 180.0 / M_PI);

    publish_control_commands(desired_heading_deg, next_target_wp.depth,
                             desired_travel_speed_);
  }
  ...
  bool load_waypoints() {
    ...
    try {
      ...
      if (!mission_yaml["waypoints"]) {
        RCLCPP_ERROR(this->get_logger(),
                     "YAML file missing 'waypoints' section.");
        return false;
      }
