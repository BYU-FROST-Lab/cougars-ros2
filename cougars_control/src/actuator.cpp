...
class Fin {
public:
  Fin(double a, double CL, double x, double c = 0, double angle = 0,
      double rho = 1026)
  ...
  double velocity_in_rotated_plane(const Eigen::Vector3d &nu) const {
    ...
    double vy_rot = std::sqrt(std::pow(vy * std::sin(angle_rad), 2) +
                              std::pow(vz * std::cos(angle_rad), 2));
  ...
  double calculate_deflection(const Eigen::Vector3d &desired_torque,
    ...
    if (desired_torque[1] != 0 && desired_torque[2] == 0) {
      ...
      std::cerr << "Error: Only one of Ty or Tz should be non-zero"
                << std::endl;
      return 0;
    }
