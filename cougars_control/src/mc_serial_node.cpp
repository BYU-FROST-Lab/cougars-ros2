  ...
  ControlNode() : Node("mc_serial_node") {
    ...
    control_command_sub_ =
        this->create_subscription<cougars_interfaces::msg::UCommand>(
            ...
            std::bind(&ControlNode::controlCommandCallback, this,
                      std::placeholders::_1));
    ...
    leak_pub_ = this->create_publisher<sensor_msgs::msg::FluidPressure>(
        "leak/data", 10);
    ...
    // This function finds the real path within the udev rule so that it can use
    // the real path with the libserialport library libserial port library
    // cannot use udev paths
    ...
    timer_ =
        this->create_wall_timer(std::chrono::milliseconds(10),
                                std::bind(&ControlNode::readSerialData, this));
  }
...
private:
  void controlCommandCallback(
      const cougars_interfaces::msg::UCommand::SharedPtr msg) {
    std::stringstream ss;
    // (Currently have 3 fins)
    ss << "$CONTR," << msg->fin[0] << "," << msg->fin[1] << "," << msg->fin[2]
       << "," << msg->thruster << "\n";
  ...
  void readSerialData() {
    ...
    int bytes_read =
        sp_nonblocking_read(serial_port_, temp_buffer, sizeof(temp_buffer) - 1);
  ...
  void processMessage(const std::string &message) {
    if (message.rfind("$DEPTH", 0) == 0) {
      ...
      if (sscanf(message.c_str(), "$DEPTH,%lf,%lf", &pressure, &temperature) ==
          2) {
        ...
        if (!this->get_parameter("demo_mode").as_bool()) {
          ...
          // std::cout << "Demo mode: Pressure not published, value: " <<
          // pressure << std::endl;
          RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                               "Demo mode: Pressure not published, value: %f",
                               pressure);
        }
      }
    }
