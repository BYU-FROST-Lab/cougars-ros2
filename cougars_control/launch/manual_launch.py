...
def generate_launch_description():
    ...
    demo_launch_arg = DeclareLaunchArgument("demo", default_value="False")
    ...
    demo = IncludeLaunchDescription(
        ...
        condition=IfCondition(
            PythonExpression(
                [LaunchConfiguration("sim"), " and ", LaunchConfiguration("demo")]
            )
        ),
    )
    ...
    launch_actions.extend(
        [
            launch.actions.IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(localization_package_dir, "converters_launch.py")
                )
            ),
