...
public:
  ComsNode() : Node("cougars_coms") {
    ...
    this->base_station_beacon_id_ =
        this->get_parameter("base_station_beacon_id").as_int();

    // buffer between immediate response to localization requests and secondary
    // packet response
    this->declare_parameter<int>("localization_queue_buffer_ms", 1000);
    this->localization_queue_buffer =
        this->get_parameter("localization_queue_buffer_ms").as_int();
    ...
    this->collision_guard_wait_ =
        this->get_parameter("collision_guard_wait_sec").as_double();

    // subscriber for safety status messages
    this->safety_subscriber_ =
        this->create_subscription<cougars_interfaces::msg::SystemStatus>(
            "safety_status", 10,
            std::bind(&ComsNode::safety_callback, this, _1));

    // subscriber for battery data
    this->battery_subscriber_ =
    ...
    this->depth_subscriber_ = this->create_subscription<
    ...
    this->dvl_subscriber_ = this->create_subscription<dvl_msgs::msg::DVLDR>(
        "dvl/position", 10, [this](dvl_msgs::msg::DVLDR msg) {
    ...
    this->modem_subscriber_ =
        this->create_subscription<seatrac_interfaces::msg::ModemRec>(
            "modem_rec", 10, std::bind(&ComsNode::listen_to_modem, this, _1));

    // publisher for sending messages via the modem
    this->modem_publisher_ =
        this->create_publisher<seatrac_interfaces::msg::ModemSend>("modem_send",
                                                                   10);

    // client for thruster control service
    this->thruster_client_ =
        this->create_client<std_srvs::srv::SetBool>("arm_thruster");

    // client for surface control service
    this->surface_client_ =
        this->create_client<std_srvs::srv::SetBool>("surface");

    // publisher for full localization data
    this->localization_data_publisher_ =
        this->create_publisher<cougars_interfaces::msg::LocalizationData>(
            "localization_data", 10);

    // publisher for short localization data
    this->localization_data_short_publisher_ =
        this->create_publisher<cougars_interfaces::msg::LocalizationDataShort>(
            "localization_data_short", 10);
    ...
    last_modem_msg_time_ =
        this->now() - rclcpp::Duration::from_seconds(10); // 10s ago
  }
  ...
  void listen_to_modem(seatrac_interfaces::msg::ModemRec msg) {
    this->last_modem_msg_time_ =
        this->now(); // Update the last modem message time
  ...
  void safety_callback(cougars_interfaces::msg::SystemStatus msg) {
    ...
    this->safety_mask = (msg.depth_status.data ? 1 << 0 : 0) |
  ...
  void kill_thruster() {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "Received kill command from base station");
    ...
    auto result_future = this->thruster_client_->async_send_request(
        request, [this](rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture
                            response_future) {
          try {
            auto response = response_future.get();
            if (response->success) {
              RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                          "Thruster has been deactivated.");
            } else {
              RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
                           "Failed to deactivate thruster.");
            }
            ConfirmEmergencyKill msg;
            msg.success = response->success;
            this->send_acoustic_message(base_station_beacon_id_, 1,
                                        (uint8_t *)&msg);
          } catch (const std::exception &e) {
            RCLCPP_ERROR(this->get_logger(),
                         "Error while trying to deactivate thruster: %s",
                         e.what());
          }
        });
  }
  ...
  void emergency_surface() {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "Received surface command from base station");
    auto request = std::make_shared<std_srvs::srv::SetBool::Request>();
    request->data = true;
    while (!this->surface_client_->wait_for_service(1s)) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                  "surface service not available, waiting again...");
    }

    auto result_future = this->surface_client_->async_send_request(
        request, [this](rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture
                            response_future) {
          try {
            auto response = response_future.get();
            if (response->success) {
              RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                          "Surface command was successfull.");
            } else {
              RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
            ...
            RCLCPP_ERROR(this->get_logger(),
                         "Error while trying to send surface command: %s",
                         e.what());
          }
        });
  }
  ...
  void send_status() {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "Sending status to base station.");
    ...
    send_acoustic_message(base_station_beacon_id_, sizeof(status_msg),
                          (uint8_t *)&status_msg);
  }
  ...
  void record_localization_info(seatrac_interfaces::msg::ModemRec msg) {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
                "Received localization info from vehicle %d.", msg.src_id);

    if (msg.packet_len < sizeof(LocalizationInfo)) {
      RCLCPP_ERROR(this->get_logger(),
                   "packet_data too small for LocalizationInfo! Got %u bytes, "
                   "expected %zu",
    ...
    const LocalizationInfo *info =
        reinterpret_cast<const LocalizationInfo *>(msg.packet_data.data());
    ...
    RCLCPP_INFO(this->get_logger(),
                "Localization Info for vehicle %d: (x, y, z): (%.2f, %.2f, "
                "%.2f), (roll, pitch, yaw): (%.2f, %.2f, %.2f), depth: %.2f, "
                "range: %.2f, azimuth: %.2f",
                localization_data.vehicle_id, localization_data.x,
                localization_data.y, localization_data.z,
                localization_data.roll, localization_data.pitch,
                localization_data.yaw, localization_data.depth,
                localization_data.range, localization_data.azimuth);
  }

  // checks if the range and azimuth data is included in the message and
  // publishes it if so
  void record_range_and_azimuth(seatrac_interfaces::msg::ModemRec msg) {
    if (msg.includes_range && msg.includes_usbl) {
      RCLCPP_INFO(this->get_logger(),
                  "Vehicle %d:  Range distance: %d, Azimuth: %i, Elevation: %i",
                  msg.src_id, msg.range_dist, msg.usbl_azimuth,
                  msg.usbl_elevation);
      ...
      this->localization_data_short_publisher_->publish(
          localization_data_short);
    }
  }

  void queue_localization_response() {
    if ((this->now() - this->last_modem_msg_time_).seconds() >=
        this->collision_guard_wait_) {
      ...
      send_acoustic_message(0, sizeof(message), (uint8_t *)&message,
                            CID_DAT_QUEUE_SET);

      RCLCPP_DEBUG(this->get_logger(), "[%ld] Queued localization data",
...
private:
  rclcpp::Subscription<seatrac_interfaces::msg::ModemRec>::SharedPtr
      modem_subscriber_;
  rclcpp::Subscription<cougars_interfaces::msg::SystemStatus>::SharedPtr
      safety_subscriber_;
  rclcpp::Subscription<sensor_msgs::msg::BatteryState>::SharedPtr
      battery_subscriber_;
  rclcpp::Subscription<sensor_msgs::msg::FluidPressure>::SharedPtr
      pressure_subscriber_;
  rclcpp::Subscription<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr
      depth_subscriber_;
  rclcpp::Subscription<dvl_msgs::msg::DVLDR>::SharedPtr dvl_subscriber_;

  rclcpp::Publisher<cougars_interfaces::msg::LocalizationData>::SharedPtr
      localization_data_publisher_;
  rclcpp::Publisher<cougars_interfaces::msg::LocalizationDataShort>::SharedPtr
      localization_data_short_publisher_;

  rclcpp::Publisher<seatrac_interfaces::msg::ModemSend>::SharedPtr
      modem_publisher_;
