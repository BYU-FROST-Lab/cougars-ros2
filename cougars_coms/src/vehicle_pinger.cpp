...
public:
  VehiclePinger() : Node("vehicle_pinger") {
    ...
    this->declare_parameter<std::vector<int64_t>>("vehicles_in_mission",
                                                  {1, 2});
    ...
    this->ping_frequency =
        this->get_parameter("ping_frequency_seconds").as_int();
    ...
    if (enable_ping) {
      this->modem_publisher_ =
          this->create_publisher<seatrac_interfaces::msg::ModemSend>(
              "modem_send", 10);
      timer_ = this->create_wall_timer(
          std::chrono::seconds(ping_frequency),
          std::bind(&VehiclePinger::ping_schedule, this));
    }
  }

  // iterates through the list of vehicles and sends a ping to next vehicle
  // every time called
  void ping_schedule() {
    vehicle_id_index += 1;
