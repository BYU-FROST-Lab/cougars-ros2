...
class Plotter:
    def __init__(self):
        ...
        (self.line,) = self.ax.plot(
            [], [], "r-", alpha=0.7
        )  # Line object for delta measurements
        (self.gps_line,) = self.ax.plot(
            [], [], "b-", alpha=0.7
        )  # Line object for GPS measurements
        (self.imu_line,) = self.ax.plot(
            [], [], "p-", alpha=0.7
        )  # Line object for delta measurements
        (self.depth_line,) = self.ax.plot(
            [], [], "y-", alpha=0.7
        )  # Line object for GPS measurements

        # Plot circles at each point
        self.delta_scatter = self.ax.scatter(
            [], [], color="red", alpha=0.4, s=250
        )  # Larger red circles
        ...
        if sensor == "gps":
            ...
            if posekey is not None:
                try:
                    ...
                    line = self.ax.plot(
                        [self.gps_timestamps[-1], self.timestamps[idx]],
                        [self.gps_values[-1], self.values[idx]],
                        "g--",
                    )[0]
            ...
            if posekey is not None:
                try:
                    idx = self.pose_keys.index(posekey)
                    line = self.ax.plot(
                        [self.depth_timestamps[-1], self.timestamps[idx]],
                        [self.depth_values[-1], self.values[idx]],
                        "y--",
                    )[0]
            ...
            if posekey is not None:
                try:
                    idx = self.pose_keys.index(posekey)
                    line = self.ax.plot(
                        [self.imu_timestamps[-1], self.timestamps[idx]],
                        [self.imu_values[-1], self.values[idx]],
                        "r--",
                    )[0]
    ...
    def update_plot(self):
        ...
        self.depth_scatter.set_offsets(
            list(zip(self.depth_timestamps, self.depth_values))
        )
